...
void cal(double x, double y, double theta_max, treenode *node, double* sum_x, double* sum_y){
...
int main(int argc, const char *argv[])
{
  ...
  for (int t = 0; t < nsteps; t++)
  {
    ...
    for (int i = 0; i < N; i++)
    {
      buf[i*6 + 0] = result[i*6 + 0];
      buf[i*6 + 1] = result[i*6 + 1];
      buf[i*6 + 3] = result[i*6 + 3];
      buf[i*6 + 4] = result[i*6 + 4];
    }
