# Script for checking A5.tar.gz file for Assignment 3 subimssion
...
echo Copying A5.tar.gz file into tmpdir_for_checking directory

cp A5.tar.gz tmpdir_for_checking || exit 1

# cd into the tmpdir_for_checking directory
cd tmpdir_for_checking || exit 1

echo Unpacking the A5.tar.gz file
tar -xzf A5.tar.gz || exit 1
...
echo Checking that running galsim executable works using 1 thread
./galsim 10 ../../input_data/ellipse_N_00010.gal 200 1e-5 0 0 1 || exit 1
...
echo Checking that running galsim executable works also for N=500 using 1 thread
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0 0 1 || exit 1
...
echo Running with a large theta_max value to check that the pos_maxdiff becomes large then, using 1 thread
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.4 0 1 || exit 1
...
echo Running with a theta_max=0.1 to check that the pos_maxdiff becomes reasonable then, using 1 thread
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.1 0 1 || exit 1
...
echo Saving result.gal as result.gal.saved
mv result.gal result.gal.saved || exit 1

echo Running the same thing using 2 threads
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.1 0 2 || exit 1

echo Using compare_gal_files program to check result.gal file, verify that diff is very small compared to previous run
./compare_gal_files 500 result.gal result.gal.saved > tmp.txt || exit 1
cat tmp.txt || exit 1
grep pos_maxdiff tmp.txt | grep 00000000 || exit 1

echo Running the same thing using 3 threads
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.1 0 3 || exit 1
...
echo Running the same thing using 4 threads
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.1 0 4 || exit 1
...
echo Running the same thing using 5 threads
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.1 0 5 || exit 1
...
echo Running the same thing using 6 threads
./galsim 500 ../../input_data/ellipse_N_00500.gal 200 1e-5 0.1 0 6 || exit 1
...
echo
echo Congratulations, your A5.tar.gz file seems OK!
echo
