...
void cal_force(double x, double y, double theta_max, treenode *node, double* sum_x, double* sum_y){
...
void *calculation(void *arg){
  ...
  double pos_dx = *(data->pos_dx);
  double pos_dy = *(data->pos_dy);
  double vel_dx = *(data->vel_dx);
  double vel_dy = *(data->vel_dy);
  double theta_max = data->theta_max;
  double G = data->G;
  ...
  for (int t = 0; t < nsteps; t++)
  {
    ...
    for (int i = 0; i < N; i += n_threads)
    {
      for (int t = 0; t < n_threads; t++){
        ...
        data->pos_dx = &(buf[j * 6 + 0]);
        data->pos_dy = &(buf[j * 6 + 1]);
        data->vel_dx = &(buf[j * 6 + 3]);
        data->vel_dy = &(buf[j * 6 + 4]);
        data->theta_max = theta_max;
        data->G = G;
        data->delta_t = delta_t;
        data->tree = tree;

        pthread_create(&thread[t], &attr, calculation, (void *)data);
